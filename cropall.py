...
# Modded by spcmd
# Original version by: https://github.com/pknowles/cropall
...
class MyApp(Tk):
	...
	def __init__(self):
		...
		self.buttons += [Button(self.controls, text="Prev [B]", command=self.previous)]
		self.buttons[-1].grid(row=0, column=4, sticky="nsew")
		self.buttons += [Button(self.controls, text="Next [N]", command=self.next)]
		...
		self.buttons += [Button(self.controls, text="Crop [Space]", command=self.save_next)]
		...
		self.bind('n', self.next)
		self.bind('b', self.previous)
